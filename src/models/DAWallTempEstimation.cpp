...
namespace JSBSim
{
  ...
  {
    const double chord_m = _chord * fttom;
    machSpeed = _machSpeed < 0.0001 ? 0.0001 : _machSpeed;
    SetInputs();
    velocity_ms = machSpeed * soundSpeed_ms;
    const double reynoldsNumber = velocity_ms * chord_m / kinematicViscosity;
    double C;
    double N;
    ...
    NN = 1 - N * (w + 1);
    return KelvinToCelsius(DAWallTempEstimation::NewtonRaphson(1E-1));
  }

  void DAWallTempEstimation::SetInputs()
  {
    airTemp_K = RankineToKelvin(atmosphere_->GetTemperature());
    airPressure_Pa = atmosphere_->GetPressure() * psftopa;
    kinematicViscosity = atmosphere_->GetKinematicViscosity() * pow(fttom, 2);
    absoluteViscosity = atmosphere_->GetAbsoluteViscosity() / (kgtoslug * fttom);
    soundSpeed_ms = atmosphere_->GetSoundSpeed() * fttom;
  }
  ...
  double DAWallTempEstimation::NewtonRaphson(double dx)
  {
