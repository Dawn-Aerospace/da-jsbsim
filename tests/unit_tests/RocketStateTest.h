...
private:
    FGFDMExec setupTest() {
        cout<<"\n-----------------------------"<<endl;
        std::string aircraft_path_ = "/home/caleb/repos/jsbsim-state/externals/vehicle-models";
        std::string model_name_ = "damk2-rocket";
        std::string scene_path_ = "/home/caleb/repos/jsbsim-state/externals/vehicle-models/damk2-rocket/NZGT_ALT.xml";
...
public:
    ...
    void TestSafeToBiPropViaMono() {
        ...
        while (FDMExec.GetSimTime() <10) {
            ...
            if(0 < FDMExec.GetSimTime()  && FDMExec.GetSimTime() <5) {
                FDMExec.SetPropertyValue("propulsion/engine/rocket-state",5);
            } else if(5 < FDMExec.GetSimTime()  && FDMExec.GetSimTime() <6) {
                FDMExec.SetPropertyValue("propulsion/engine/rocket-state",0);
            } else if(6 < FDMExec.GetSimTime()  && FDMExec.GetSimTime() <10) {
                FDMExec.SetPropertyValue("propulsion/engine/rocket-state",1);
            }
        }
        TS_ASSERT_EQUALS(FDMExec.GetPropertyValue("propulsion/engine/rocket-state"),1);
